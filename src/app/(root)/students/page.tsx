...
export default function Page() {
  ...
  const getStudentData = async () => {
    try {
      const { data } = await axios.get(`/students/${searchParams.get("id")}`);
      console.log("Student data:", data);
      dispatch(setBasicInfoUserData(data.basicInfo.user));
      dispatch(setBasicInfoStudentData(data.basicInfo.student));
  ...
  const handleNext = (data: any) => {
    switch (step) {
      ...
      case 5:
        ...
        handleRegistrationSubmit(); // Call API after completing medical info
        return;
      default:
        break;
    }
  ...
  const handleRegistrationSubmit = async () => {
    const apiUrl = "http://16.170.155.154:3300/api/students/";

    // Check if registrationData.basicInfo and registrationData.basicInfo.student are defined before accessing them
    if (!registrationData.basicInfo || !registrationData.basicInfo.student) {
      console.error("Basic info or student info is not defined.");
      return;
    }

    // Convert rollNumber to integer if it's supposed to be an integer
    const basicInfo = {
      user: { ...registrationData.basicInfo.user },
      student: {
        ...registrationData.basicInfo.student,
        rollNumber: parseInt(registrationData.basicInfo.student.rollNumber, 10), // Convert to integer
      },
    };
    ...
    const payload = {
      basicInfo,
      addressInfo: { ...registrationData.addressInfo },
      parentInfo: { ...registrationData.parentInfo },
        ...
        <div className="px-10 pb-8 ">
          ...
          <div className="2xl:mt-10 xl:mt-6 flex justify-between">
            <button
              ...
              className={`${
                step >= 6 ? "pointer-events-none opacity-50" : ""
