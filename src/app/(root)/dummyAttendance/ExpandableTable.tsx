              ...
              >
                ...
                {sortColumn === column.key && (
                  <span className="relative inline-block ml-1">
                    {sortDirection === "asc" ? (
                      <ChevronUp size={15} className="absolute -bottom-0.5" />
                    ) : (
                      <ChevronDown size={15} className="absolute -bottom-0.5" />
                    )}
                  </span>
                )}
              </th>
              ...
              <React.Fragment key={row.id}>
                ...
                >
                  {columns.map((column) => (
                    <td
                      key={column.key}
                      className={`py-4 px-8 border-b text-gray-500 ${
                        column.key === "absent"
                          ? row[column.key] === "absent"
                            ? "text-green-500"
                            : "text-red-500"
                          : ""
                      }`}
                                  ...
                                  {subColumns.map((subColumn) => (
                                    <td
                                      ...
                                      className={`py-4 px-8 border-b text-left capitalize  ${
                                        subColumn.key === "status"
                                          ? subRow[subColumn.key] === "present"
                                            ? "text-green-500"
                                            : "text-red-500 "
                                          : ""
                                      }`}
