    ...
    sortedStages.forEach((stage) => {
      if (stage.stage === "Pre Primary Stage") {
        ...
        stage.classes.sort((a, b) => {
          return (
            prePrimaryOrder.indexOf(a.name) - prePrimaryOrder.indexOf(b.name)
          );
        });
      } else {
    ...
    <>
      {isLoading ? (
        <div className="flex flex-col items-center justify-center min-h-screen ">
          ...
          <Progress value={progress} className="w-80 h-2 bg-lmsAccent" />
          ...
          <div className="h-20 flex items-center justify-between border-b border-lms-100 px-16">
            ...
            <div>
              {!(constantStages.length === stages.length) && (
                <MultiSelectCardGrid fetchData={fetchData} stages={stages} />
              )}
            </div>
          </div>
