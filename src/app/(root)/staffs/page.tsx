...
export default function Page() {
	...
	const setStaffData = useCallback(
		(staffData: Staff) => {
			...
			dispatch(
				setPreviousExperienceData(staffData.previousExperienceInfo)
			);
		...
		if (step === 1) {
			...
			try {
				if (viewState === "view" || viewState === "edit") {
					await axios.put(
						`http://16.170.155.154:3300/api/staffs/${searchParams.get(
							"id"
						)}`,
			...
			<div className='h-[590px] w-full rounded-2xl bg-white shadow-custom-dark 2xl:mt-12 xl:mt-6 flex flex-col'>
				...
				<div className='flex-grow px-10 overflow-y-auto'>
					{loading || status === "loading" ? (
						...
						<>
							...
							{step === 2 && (
								<EmploymentDetails onNext={handleNext} />
							)}
							{step === 3 && <AddressInfo onNext={handleNext} />}
							{step === 4 && (
								<AdditionalDetails onNext={handleNext} />
							)}
							{step === 5 && (
								<PreviousExperience onNext={handleNext} />
							)}
				...
				<div className='px-10 pb-10 flex justify-between items-end'>
					...
					{viewState === "view" || viewState === 'edit' ? (
						<Switch
							checkedChildren="view" unCheckedChildren="edit"
