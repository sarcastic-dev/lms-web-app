...
const DynamicTabs: React.FC<DynamicTabsProps> = ({
      ...
      if (step === 1) {
        ...
        const modifiedRegistrationData = {
          ...
          medicalInfo: {
            ...
            haemoglobin: Number(registrationData.medicalInfo.haemoglobin),
          },
        };
  ...
  const isTabCompleted = (index: number) => completedSteps.includes(index + 1);
      ...
      if (isEdit) {
        if (id && userType) {
          getUserData({ id, userType })
            .then((data) => {
              console.log(data);
              dispatch(setBasicInfoUserDetailData(data.basicInfo.user));
              dispatch(setBasicInfoStaffData(data.basicInfo.staff));
              dispatch(setStaffAddress(data.addressInfo));
              dispatch(setAdditionalDetailsData(data.additionalInfo));
              dispatch(setPreviousExperienceData(data.previousExperienceInfo));
        ...
        {config.map((tab, index) => {
          ...
          return (
            <TabsTrigger
              ...
              value={tab.value}
              className={`${
                isActive || isComplete ? "border-t-lmsSuccess" : ""
              }`}
            >
              <span>
                {isComplete ? (
              ...
              <span className={`${isActive ? "text-lmsAccent" : ""}`}>
                {tab.label}
              </span>
            </TabsTrigger>
          );
