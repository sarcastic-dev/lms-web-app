      ...
      {showProfileCreation ? (
        ...
        <>
          <div className="bg-white border p-8 shadow-xl w-2/6 h-3/6 rounded-lg z-10">
            <Image src='/dummyIcon.png' alt='companyImage' width={200} height={200} className='ml-28' />
            <div className="flex flex-col items-center">
              <form onSubmit={hasAccount ? handlePasswordSubmit : handleSubmit} className="w-full flex flex-col items-center">
                {hasAccount ? (
                  <div className="relative w-96 mb-4 mt-6">
                    <label className={`absolute left-5 top-4 text-gray-400 font-medium transition-all duration-200 ease-in-out ${isPasswordFocused || password ? 'text-xs -top-1 mt-3' : ''}`}>
                      Password
                    </label>
                    <input
                      type="password"
                  ...
                  <div className="relative w-96 mb-4 mt-6">
                    <label className={`absolute left-5 top-4 text-gray-400 font-medium transition-all duration-200 ease-in-out ${isFocused || input ? 'text-xs -top-1 mt-3' : ''}`}>
                ...
                <button
                  type="submit"
                  className={`w-96 py-2 border-2 font-semibold border-blue-500 rounded-xl mb-8 ${isButtonDisabled ? 'text-blue-500 cursor-not-allowed' : 'bg-blue-500 text-white hover:bg-blue-600'}`}
