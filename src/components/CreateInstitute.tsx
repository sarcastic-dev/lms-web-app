...
interface FormErrors {
  ...
  phoneNumber?: string;
  general?: string;
  instituteAddress?: string; // Added instituteAddress key
}
...
const CreateInstitute: React.FC = () => {
  ...
  const [instituteAddress, setInstituteAddress] = useState(""); // New state for institute address
  ...
  useEffect(() => {
    if (
      ...
      city.trim() &&
      instituteType.trim() &&
      instituteAddress.trim() // Check for instituteAddress
    ) {
      setIsButtonDisabled(false);
    } else {
      setIsButtonDisabled(true);
    }
  }, [instituteName, address, phoneNumber, city, instituteType, instituteAddress]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      const instituteData = {
        ...
        city,
        instituteType,
        instituteAddress, // Add instituteAddress to payload
      };
      ...
      try {
        ...
        setErrors({ ...errors, general: "Failed to create institute." }); // Update the state with general error
        setIsCreating(false);
      }
    }
  };
      ...
      <div className="relative mb-4">
        ...
        <div className="relative mb-4">
          <input
            ...
            required
          />
          <div className={getLabelClasses("address", address)}>Email Address</div>
        ...
        <div className="relative mb-4">
          <input
            ...
            required
          />
          <div className={getLabelClasses("instituteAddress", instituteAddress)}>Institute Address</div>
          {errors.instituteAddress && <p className="text-red-500 text-sm mt-1">{errors.instituteAddress}</p>}
        </div>
